...
output:
  html_document:
    ...
    df_print: paged
params:
  data_path: "/gpfs/data/aifantislab/home/nadorb01/CITEseq_cellranger_v5/2021-03-09/count-Audrey_cDNA1/" # path to the directory with the outs file
  out_path: "/gpfs/data/aifantislab/home/nadorb01/projects/ad_ped_AML_v5/data/pediatric" # output path where the outputs will be stored
  sample_name: "2021-03-09-count1_0.95"   # name of the current sample 
  hashtag_ID: ["hashtag1", "hashtag2", "hashtag3", "hashtag4"] # names of the hashtags to keep (if the hashtags are NOT separate from the gene expression data)
  hashtags_to_keep: ["hashtag1", "hashtag2", "hashtag3", "hashtag4"] # names of the hashtags to keep (if the hashtags are NOT separate from the gene expression data)
  hashtags_dx: ["hashtag1", "hashtag3"]
...
### SCRIPT ADJUSTED FOR ADDITIONAL HASHATG THAT IS NOT PRESENT IN OUR DATA
attach(params) 
...
if(!is.null(hashtag_ID) | !is.null(hashtag_file)) {
  ...
  seurat_obj <- HTODemux(seurat_obj, assay = "HTO", positive.quantile = 0.95)
...
```{r filter_dx}
if(!is.null(hashtags_dx)) {
  seurat_obj <- subset(seurat_obj,
                       cells = WhichCells(seurat_obj,
                                          idents = hashtags_dx))
saveRDS(seurat_obj, file = paste0(out_path, "/", "seurat_obj_Dx.rds"))
}
