...
output:
  html_document:
    ...
    df_print: paged
params:
  data_path: "/gpfs/data/sequence/results/aifantislab/2021-runs/2021-03-09/cellranger/count-library1_2_cDNA/" # path to the directory with the outs file
  out_path: "/gpfs/data/aifantislab/home/nadorb01/projects/ad_ped_AML_v5/data/adult" # output path where the outputs will be stored
  sample_name: "2021-03-09-Z-1-2"   # name of the current sample 
  hashtag_ID: ["hashtag1", "hashtag2", "hashtag3", "hashtag4", "hashtag5", "hashtag6"] # names of the hashtags to keep (if the hashtags are NOT separate from the gene expression data)
  hashtags_to_keep: ["hashtag1", "hashtag2", "hashtag3", "hashtag4", "hashtag5", "hashtag6"] # names of the hashtags to keep (if the hashtags are NOT separate from the gene expression data)
  ...
  emily: FALSE # logical ADT name recovery
  reduced_panel: TRUE # logical ADT name recovery - go through emily translation
---
...
if(ADT) {
# this has an additional hash tag without cells - remove hashtag7
hash_idx <- which(rownames(counts_mat[["Antibody Capture"]]) %in% c(interect_hash, "hashtag7"))
ADT_counts <- counts_mat[["Antibody Capture"]][-hash_idx, ]


# # make reference file that automatically translates to christian:
# reference <- read.csv("/gpfs/data/aifantislab/home/nadorb01/Common_data/ADT_panel_match.csv", stringsAsFactors=F)
# reference1 <- read.csv("/gpfs/data/aifantislab/home/nadorb01/Common_data/emily_christian_adt_match.csv", stringsAsFactors=F)

# # split to numbers in each file and match
# reference1$AB <- str_split_fixed(reference1$old, "_", 2)[,1]
# reference$AB <- str_split_fixed(reference$old_panel, "_", 2)[,1]

# #  fix rownames!
# master <- merge(reference, reference1, by="AB", all.x=TRUE, all.y=FALSE)
# master$old <- ifelse(is.na(master$old), master$old_panel,  master$old)
# write.csv(master[,c("new_panel", "old")], "/gpfs/data/aifantislab/home/nadorb01/Common_data/ADT_panel_old_match.csv", row.names=F)

if(reduced_panel){
    reference <- read.csv("/gpfs/data/aifantislab/home/nadorb01/Common_data/ADT_panel_old_match.csv", stringsAsFactors=F, row.names=1)

    ADT_counts <- ADT_counts %>% 
                    as.data.frame() 

    ADT_counts <- merge(reference, ADT_counts, by=0) %>%
                     column_to_rownames("old") %>% 
                     select(-Row.names) %>%
                     as.matrix()
}
...
if(emily){
    reference <- read.csv("/gpfs/data/aifantislab/home/nadorb01/Common_data/emily_christian_adt_match.csv", stringsAsFactors=F, row.names=1)

    ADT_counts <- ADT_counts %>% 
                    as.data.frame() 

    ADT_counts <- merge(reference, ADT_counts, by=0, all.x=FALSE, all.y=TRUE) %>%
                     column_to_rownames("old") %>% 
                     select(-Row.names) %>%
                     as.matrix()
}
...
print(seurat_obj@meta.data %>% select(nCount_ADT, samples) %>% group_by(samples) %>% mutate(mean_count=mean(nCount_ADT)) %>% select(mean_count, samples) %>% unique())
