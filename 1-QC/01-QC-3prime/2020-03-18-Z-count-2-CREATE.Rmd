...
output:
  html_document:
    ...
    df_print: paged
params:
  data_path: "/gpfs/data/aifantislab/home/nadorb01/CITEseq_cellranger_v5/2020-03-18/count-Zhengxi2/" # path to the directory with the outs file
  ...
  sample_name: "2020-03-18-AML0048-c2"   # name of the current sample 
  hashtag_ID: ["Dx_TotalSeqA",  "PR_TotalSeqA", "R1_TotalSeqA", "R2_TotalSeqA"] # names of the hashtags to keep (if the hashtags are NOT separate from the gene expression data)
  hashtags_to_keep: ["Dx_TotalSeqA",  "PR_TotalSeqA", "R1_TotalSeqA", "R2_TotalSeqA"] # names of the hashtags to keep (if the hashtags are NOT separate from the gene expression data)
...
if(!is.null(hashtag_ID)) {
  ...
  # filter cells that have no HTO counts
  counts_mat <- subset(counts_mat, colSums(counts_mat[["Antibody Capture"]][hash_idx, ]) > 0 )
  ...
  if(ADT) {
    ...
    ADT_names <- apply(as.data.frame(rownames(ADT_counts)) ,1, function(x) str_split(x, "-")[[1]][1]) %>% 
      as.data.frame() %>% 
      mutate(ADT=gsub("_TotalSeqA", "", ADT)) %>%
      mutate(ADT = toupper(.)) %>% 
...
if(!is.null(hashtag_file)) {
  ...
  # filter cells that have no HTO counts
  hto_counts <- subset(hto_counts, colSums(hto_counts) > 0 )
